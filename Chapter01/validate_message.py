...
def fetch_public_key(user: bytes):
...
## anything other message will result in an invalid signature
...
## 'b' make a string a seuqence of octets
user = b"nelson"

# fetch public key from Nelson
public_key = fetch_public_key(user)
# … verify the message like before
## if message and signature can be verified by the public key, there will be no exception
## otherwise, any difference in either will result in an exception
public_key.verify(
    signature,
    message,
